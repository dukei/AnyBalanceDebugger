...
class AsyBalance {
    ...
    async api_retrieveCode(comment, image, options) {
        ...
        let _options = '';
        let _imagestr = typeof image === 'string' ? image : '';
        if (__classPrivateFieldGet(this, _global).api_stringified) {
            if (options)
                _options = JSON.stringify(options);
            if (image && !_imagestr)
                _imagestr = base64_arraybuffer_1.encode(image);
        }
        if (__classPrivateFieldGet(this, _global).apiRetrieve)
            return this.checkCallResponse(await __classPrivateFieldGet(this, _global).apiRetrieve.retrieveCode(comment, _imagestr || image, _options || options));
        _imagestr = _imagestr || base64_arraybuffer_1.encode(image);
        return this.callAnyBalance(method, [comment, _imagestr, options]);
    }
    async api_loadData() {
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.HTTP_METHOD = exports.OPTIONS = exports.RetrieveInputType = void 0;
